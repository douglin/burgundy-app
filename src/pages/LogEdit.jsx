...
export default function LogEdit() {
  ...
  const { entries, updateEntry } = useLog()
  const entry = entries.find(e => e.id === id)

  const [form, setForm] = useState(() => entry ? { ...entry } : null)
  const [photoPreview, setPhotoPreview] = useState(entry?.photoPath || null)
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-12">
      <p className="text-[#C9A84C] text-xs tracking-[0.3em] uppercase mb-1">Edit</p>
      <h1
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <fieldset className="border border-[#D4C5A9] p-5">
          <legend className="text-xs tracking-widest uppercase text-[#6B5244] px-2">Cellar</legend>
          <div className="grid grid-cols-3 gap-4 mt-2">
            <label className="block">
              ...
              <input
                type="number"
                min="0"
                value={form.cellar?.bottlesOwned ?? 0}
        ...
        <div className="flex gap-3">
          <button
            type="submit"
            className="flex-1 bg-[#6B0F1A] text-[#F5F0E8] py-3 text-xs tracking-widest uppercase hover:bg-[#4A0A12] transition-colors"
          >
            Save Changes
          </button>
          <Link
            to={`/log/${id}`}
            className="px-6 py-3 border border-[#D4C5A9] text-xs tracking-widest uppercase text-[#6B5244] hover:border-[#6B0F1A] hover:text-[#6B0F1A] transition-colors flex items-center"
          >
            Cancel
          </Link>
        </div>
